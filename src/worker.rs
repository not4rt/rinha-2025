...
async fn main() {
    ...
    loop {
        ...
        monoio::spawn(async move {
            handle_payment_stream(&mut stream.0).await;
        });
    }
}
...
async fn handle_payment_stream(stream: &mut UnixStream) {
    let mut tx = PAYMENT_SENDER.get().unwrap();
    loop {
        match stream.read(vec![0u8; 85]).await {
            (Ok(85), buffer) => {
                // POST payment
                let _ = tx
                    .send(unsafe { buffer.try_into().unwrap_unchecked() })
                    .await;
            }
            (Ok(0), _) => {
                // cold_path();
            ...
            (Ok(_), buffer) if buffer[0] == b'?' || buffer[0] == b' ' => {
                cold_path();
                // GET Summary
